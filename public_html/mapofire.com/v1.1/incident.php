...
$fire = json_decode(file_get_contents('https://api.mapotechnology.com/v1/wildfires/incident?key=50e2c43f8f63ff0ed20127ee2487f15e&history=1&wfid='.$_GET['wfid']))->fire;
...
if (!$fire->error) {
    ...
    $title = $fireName.' near '.explode(' of ', $near)[1].' - Map of Fire: Current Incident Information and Wildfire Map';
...
<?if ($fire->error) {?>
    <article class="top">
        <div class="container" style="position:relative;min-height:600px">
            <div style="position:absolute;top:50%;transform:translateY(-50%);width:100%;text-align:center;margin:0 auto">
                <h1 style="margin-bottom:1em">Not Found</h1>
                <h2>The wildland fire incident you're looking for either doesn't exist, or the URL is incorrect.</h2>
                <a class="btn btn-xl btn-gray" style="margin-top:2em" href="#" onclick="history.go(-1)">Go Back</a>
            </div>
        </div>
    </article>
<?} else{
...
<article class="top">
    <div class="container">
        ...
        <a class="btn btn-xl btn-yellow centered" style="margin-bottom:0" href="https://mapofire.com">View map of all wildfires</a>
    </div>
</article>
...
<article>
    <div class="container">
        ...
        <? // acres history
        if ($fire->properties->acres_history != null && count($fire->properties->acres_history) > 1) {?>
        <div class="fire-details">
            <h2 class="title">Acreage History</h2>
            <div class="wrapper">
                <table class="ah-table">
                    <thead>
                        <tr><th>Updated</th><th>Size</th><th>Change</th></tr>
                    </thead>
                    <tbody>
                    <?for ($i = 0; $i < count($fire->properties->acres_history); $i++) {
                        $e = $fire->properties->acres_history[$i];
                        echo '<tr><td>'.ago($e->updated).'</td><td>'.sizing($e->acres, 2).' '.strtolower(sizing(null, 1)).'</td><td>'.($e->change > 0 ? '+' : '').sizing($e->change, 2).' '.strtolower(sizing(null, 1)).'</td></tr>';
                    }?>
                    </tbody>
                </table>
            </div>
