...
function apiUrl(t, s = null) {
    return (s == null || s == 1 ? apiURL : apiURL2) + t + '?key=' + apiKey;
}

function replaceWith(element, t) {
...
function rating(t, rh, avg, max) {
    var a = (t <= 67 ? 1 : ((t > 67 && t <= 82) ? 2 : 3)),
        b = (rh < 15 ? 3 : (rh < 25 ? 2 : 1)),
        c = (avg < 10 ? 1 : (avg >= 10 && avg < 15 ? 2 : 3)),
        d = (max < 10 ? 1 : (max >= 10 && max < 15 ? 2 : 3)),
        z = (a + (b * 2) + ((c + d) * 0.75)) / 4,
...
function notify(t, m) {
    if (isAndroid == 1) {
        ...
        /* if modal is open, adjust left absolute margin */
        if (isVisible(document.querySelector('#modal'))) {
...
function ndfdLegend() {
    ...
    document.body.insertAdjacentHTML('beforeend', '<div id="ndfdLegends" style="display:none">' + o + '</div>');
...
function updateProgressBar(d = null) {
    /*var numTasks = 8,
        ...
        com = parseFloat(document.querySelector('#loading .spinner').getAttribute('data-progress')) + w;

    document.querySelector('#loading .spinner').setAttribute('data-progress', com);

    if (d != null) {
        document.querySelector('#loading span').innerHTML = d;
    }

    /* when progress bar hits 100%, hide the loading overlay *//*
    if (d == 'Wildfires loaded...') {
        setTimeout(function () {
    ...
    document.querySelector('#loading').innerHTML = '<div class="content"></div>';
    document.querySelector('#loading .spinner').setAttribute('data-progress', '0');
}, 0);
}*/
}
...
function connection(s) {
    ...
    if (s === false) {
        document.body.insertAdjacentHTML('beforeend', '<div id="overlay-modal"><div class="mes"><div class="icon"><i class="fa-regular fa-wifi-slash"></i></div><div class="text">' +
            '<h3>No Internet</h3><p>You are not connected to a network. The map won\'t be able to load any new data without a connection.</p>' +
            '<a href="#" class="btn btn-light-blue" onclick="this.parentElement.parentElement.parentElement.remove();return false;">Got it, thanks!</a></div></div></div>');
    } else {
        const n = document.querySelector('#overlay-modal');
    ...
    if (new Date(d * 1000).getFullYear() < z.getFullYear()) {
        ...
        if (t == 'Prescribed Fire') {
            ...
            if (s == 'contained' || note.search('contain') >= 0) {
                ...
                if (n - d <= (60 * 60 * hrs)) {
                    c = ' new' + (a != '' && a != 'Unknown' && a > (((n - d) / 3600) * aph) ? ' fast' : '');
                } else {
...
async function saveLogFire() {
    var a = localStorage.getItem('clks');

    if (a) {
        await fetch(apiUrl('logFire', 2), {
            method: 'POST',
            body: new FormData().append('data', a)
        }).then(() => {
...
function unsetHeaders() {
    if (window.location.href.search('fires') >= 0 || window.location.href.search('weather/') >= 0 || window.location.href.search('risk') >= 0) {
        ...
        document.querySelector('meta[name="description"]').setAttribute('content', odesc);
    }
}

function openModal() {
    document.querySelector('#modal').classList.remove('closing');
    document.querySelector('#modal').classList.add('open');
}

function closeModal() {
    document.querySelector('#modal').classList.remove('open', 'fire');
...
function openImpact(feat, title) {
    const impact = document.querySelector('#impact');

    if (impact.offsetLeft < 0) {
        impact.querySelectorAll('.wrapper > div').forEach((e) => {
            e.style.display = (e.classList.contains(feat) ? 'block' : 'none');
        });

        impact.querySelector('h2.title').innerHTML = title;
        impact.classList.add('openImpact');
        impact.classList.remove('closeImpact');
    } else {
        closeImpact();
    }
}
...
function closeImpact() {
    ...
    impact.classList.remove('openImpact');
    impact.classList.add('closeImpact');

    // remove existing wwas
    if (impact.querySelector('.wwas')) {
        impact.querySelector('.wwas').remove();
        document.querySelector('#impact').setAttribute('data-wwas', 0);
    }
    ...
    if (document.querySelector('li#report').getAttribute('data-active') == 1) {
        ...
        map.removeLayer(nrMrkr);
    }
}

function removePlaceholders() {
    document.querySelectorAll('#modal .content .container > *').forEach((t) => {
...
function myAccount(u, a) {
    ...
    if (s) {
        document.querySelector('.account').innerHTML = s;
    }

    if (u.uid) {
        const account = new User();
        document.querySelector('#sync span').innerHTML = 'Account last synced ' + (u.settings.method == 1 ? 'automatic' : 'manu') + 'ally ' + timeAgo(u.settings.synced).toLowerCase();

        // if the user has a valid subscription for premium
        /*if (account.validSubscription()) {
            const details = account.subscriptionDetails(),
                vs = '<p>Map-o-Fire Premium</p><span style="font-size:14px;color:#999">' + (details.is_trial ? 'Trial ends ' : 'Renews on ') + dateTime(details.period_end) + '</span></div>';

            document.querySelector('.account #subs').innerHTML = vs;
        } else {
            document.querySelector('.account #subs').innerHTML = '<p>No valid subscriptions</p><span><a href="https://www.mapotechnology.com/checkout?price_id=price_1MgxhSIpCdpJm6cTaKp2dqf5&host=mapofire.com&return=' + encodeURIComponent(window.location.pathname) + '">Subscribe</a></span>'
        }*/

        if (settings.perimeters) {
            document.querySelectorAll('#perimColor option').forEach((e) => {
                if (e.value == settings.perimeters.color) {
                    e.selected = true;
                }
            });

            document.querySelectorAll('#perimZoom option').forEach((e) => {
                if (e.value == settings.perimeters.zoom) {
                    e.selected = true;
                }
            });

            document.querySelectorAll('#perimTtip option').forEach((e) => {
                if (e.value == settings.perimeters.ttip) {
                    e.selected = true;
                }
            });
        }
        ...
        if (settings.weather) {
            document.querySelectorAll('#tempUnit option').forEach((e) => {
                if (e.value == settings.weather.temp) {
                    e.selected = true;
                }
            });

            document.querySelectorAll('#windSpeedUnit option').forEach((e) => {
                if (e.value == settings.weather.wind) {
                    e.selected = true;
                }
            });
        }

        document.querySelectorAll('#saveFreq option').forEach((e) => {
            if (e.value == settings.saveFreq) {
                e.selected = true;
            }
        });

        document.querySelectorAll('#locallySave option').forEach((e) => {
            if (e.value == settings.locallySave) {
                e.selected = true;
            }
        });

        document.querySelectorAll('#coordsDisplay option').forEach((e) => {
            if (e.value == settings.coordsDisplay) {
                e.selected = true;
            }
        });

        document.querySelectorAll('#fireDisplay option').forEach((e) => {
            if (e.value == settings.fireDisplay) {
                e.selected = true;
            }
        });

        document.querySelectorAll('#acresUnit option').forEach((e) => {
            if (e.value == settings.acres) {
                e.selected = true;
            }
        });
    }
...
function incidentDetails(data) {
    let a = ['Basic Information', 'Current Situation', 'Outlook', 'Weather Synopsis'],
        o = '';
...
window.addEventListener('mouseup', (e) => {
    const target = e.target,
        ...
        im = document.querySelector('#impact');

    if (isVisible(sr)) {
        if (target !== sr && !sr.contains(target) && target !== sc && !sc.contains(target)) {
    ...
    if (im.offsetLeft > 0) {
        if (target !== im && !im.contains(target) && document.querySelector('#report').getAttribute('data-active') == 0 && document.querySelector('#impact').getAttribute('data-wwas') == 0) {
            //closeImpact();
        }
    }
});
