...
function h(t, i, r) {
	var s = Math.sin(i * e),
		n = Math.pow(2, r),
		a = n * (t / 360 + .5);
	return (a %= n) < 0 && (a += n), [a, n * (.5 - .25 * Math.log((1 + s) / (1 - s)) / Math.PI), r]
}
var c = {
		...
		var t = n(e[0], e[1], 32),
			...
			s = function (e) {
				for (var t = 28, i = 0; i < t; i++) {
					var r = 1 << 32 - (i + 1);
					if ((e[0] & r) != (e[2] & r) || (e[1] & r) != (e[3] & r)) return i
				}
				return t
			}(r);
	...
	p = {
		...
		read: function (e, t, i, r, s) {
			var n, a, o = 8 * s - r - 1,
				...
				c = i ? -1 : 1,
				p = e[t + h];
			for (h += c, n = p & (1 << -d) - 1, p >>= -d, d += o; d > 0; n = 256 * n + e[t + h], h += c, d -= 8);
			for (a = n & (1 << -d) - 1, n >>= -d, d += r; d > 0; a = 256 * a + e[t + h], h += c, d -= 8);
			if (0 === n) n = 1 - u;
			else {
		...
		write: function (e, t, i, r, s, n) {
			var a, o, l, u = 8 * n - s - 1,
				...
				f = r ? 1 : -1,
				v = t < 0 || 0 === t && 1 / t < 0 ? 1 : 0;
			for (t = Math.abs(t), isNaN(t) || t === 1 / 0 ? (o = isNaN(t) ? 1 : 0, a = d) : (a = Math.floor(Math.log(t) / Math.LN2), t * (l = Math.pow(2, -a)) < 1 && (a--, l *= 2), (t += a + h >= 1 ? c / l : c * Math.pow(2, 1 - h)) * l >= 2 && (a++, l /= 2), a + h >= d ? (o = 0, a = d) : a + h >= 1 ? (o = (t * l - 1) * Math.pow(2, s), a += h) : (o = t * Math.pow(2, h - 1) * Math.pow(2, s), a = 0)); s >= 8; e[i + p] = 255 & o, p += f, o /= 256, s -= 8);
			for (a = a << s | o, u += s; u > 0; e[i + p] = 255 & a, p += f, a /= 256, u -= 8);
			e[i + p - f] |= 128 * v
		}
	},
	f = y,
...
function b(e, t, i) {
	...
	i.realloc(r);
	for (var s = i.pos - 1; s >= e; s--) i.buf[s + r] = i.buf[s]
}
...
y.prototype = {
	...
	readVarint: function (e) {
		...
		return t = 127 & (i = r[this.pos++]),
			i < 128 ? t : (t |= (127 & (i = r[this.pos++])) << 7, i < 128 ? t : (t |= (127 & (i = r[this.pos++])) << 14, i < 128 ? t : (t |= (127 & (i = r[this.pos++])) << 21, i < 128 ? t : function (e, t, i) {
				var r, s, n = i.buf;
				if (s = n[i.pos++], r = (112 & s) >> 4, s < 128) return S(e, r, t);
				if (s = n[i.pos++], r |= (127 & s) << 3, s < 128) return S(e, r, t);
				if (s = n[i.pos++], r |= (127 & s) << 10, s < 128) return S(e, r, t);
				if (s = n[i.pos++], r |= (127 & s) << 17, s < 128) return S(e, r, t);
				if (s = n[i.pos++], r |= (127 & s) << 24, s < 128) return S(e, r, t);
				if (s = n[i.pos++], r |= (1 & s) << 31, s < 128) return S(e, r, t);
		...
		return this.pos = e,
			e - t >= 12 && m ? function (e, t, i) {
				...
				for (; s < i;) {
					...
					if (s + d > i) break;
					1 === d ? l < 128 && (u = l) : 2 === d ? 128 == (192 & (n = e[s + 1])) && (u = (31 & l) << 6 | 63 & n) <= 127 && (u = null) : 3 === d ? (n = e[s + 1], a = e[s + 2], 128 == (192 & n) && 128 == (192 & a) && ((u = (15 & l) << 12 | (63 & n) << 6 | 63 & a) <= 2047 || u >= 55296 && u <= 57343) && (u = null)) : 4 === d && (n = e[s + 1], a = e[s + 2], o = e[s + 3], 128 == (192 & n) && 128 == (192 & a) && 128 == (192 & o) && ((u = (15 & l) << 18 | (63 & n) << 12 | (63 & a) << 6 | 63 & o) <= 65535 || u >= 1114112) && (u = null)),
						null === u ? (u = 65533, d = 1) : u > 65535 && (u -= 65536, r += String.fromCharCode(u >>> 10 & 1023 | 55296), u = 56320 | 1023 & u),
	...
	skip: function (e) {
		...
		else if (t === y.Bytes) this.pos = this.readVarint() + this.pos;
		else if (t === y.Fixed32) this.pos += 4;
		else {
			if (t !== y.Fixed64) throw new Error("Unimplemented type: " + t);
			this.pos += 8
		}
	},
	...
	writeVarint: function (e) {
		(e = +e || 0) > 268435455 || e < 0 ? function (e, t) {
			var i, r;
			e >= 0 ? (i = e % 4294967296 | 0, r = e / 4294967296 | 0) : (r = ~(-e / 4294967296), 4294967295 ^ (i = ~(-e % 4294967296)) ? i = i + 1 | 0 : (i = 0, r = r + 1 | 0));
			...
			t.realloc(10),
				...
				function (e, t) {
					var i = (7 & e) << 4;
					if (t.buf[t.pos++] |= i | ((e >>>= 3) ? 128 : 0), !e) return;
		...
		this.pos = function (e, t, i) {
			for (var r, s, n = 0; n < t.length; n++) {
				if ((r = t.charCodeAt(n)) > 55295 && r < 57344) {
					if (!s) {
						r > 56319 || n + 1 === t.length ? (e[i++] = 239, e[i++] = 191, e[i++] = 189) : s = r;
						continue
					}
					if (r < 56320) {
						e[i++] = 239,
					...
					r = s - 55296 << 10 | r - 56320 | 65536,
						s = null
				} else s && (e[i++] = 239, e[i++] = 191, e[i++] = 189, s = null);
				r < 128 ? e[i++] = r : (r < 2048 ? e[i++] = r >> 6 | 192 : (r < 65536 ? e[i++] = r >> 12 | 224 : (e[i++] = r >> 18 | 240, e[i++] = r >> 12 & 63 | 128), e[i++] = r >> 6 & 63 | 128), e[i++] = 63 & r | 128)
			}
			return i
		}(this.buf, e, this.pos);
		var i = this.pos - t;
...
function A(e, t) {
	...
	if (1 === i) r.coordinates.push(L(e.geometry.coords, t, 0, 2 * e.geometry.lengths[0]));
	else {
		r.type = "MultiPolygon";
		let s = 0;
		for (let n = 0; n < i; n++) {
...
function C(e) {
	for (var t, i = 0, r = 0, s = e.length, n = e[r]; r < s - 1; r++) i += ((t = e[r + 1])[0] - n[0]) * (t[1] + n[1]),
		n = t;
	return i >= 0
}
...
function $$(e, t, i) {
	for (let r = 0; r < e.length; r++) {
		if (e[r].name === i) return t[r][t[r].value_type]
	}
	return null
}
...
function W(e, t) {
	let i = e[0],
		r = e[1],
		s = e[2] ? e[2] : void 0;
	t.scale && (i *= t.scale.xScale, r *= -t.scale.yScale, void 0 !== s && (s *= t.scale.zScale)),
		t.translate && (i += t.translate.xTranslate, r += t.translate.yTranslate, void 0 !== s && (s += t.translate.zTranslate));
	const n = [i, r];
...
j.read = function (e, t) {
	...
	j.Feature._readField = function (e, t, i) {
		1 === e ? t.attributes.push(j.Value.read(i, i.readVarint() + i.pos)) : 2 === e ? (t.geometry = j.Geometry.read(i, i.readVarint() + i.pos), t.compressed_geometry = "geometry") : 3 === e ? (t.shapeBuffer = j.esriShapeBuffer.read(i, i.readVarint() + i.pos), t.compressed_geometry = "shapeBuffer") : 4 === e && (t.centroid = j.Geometry.read(i, i.readVarint() + i.pos))
	},
	j.Feature.write = function (e, t) {
		if (e.attributes)
			for (var i = 0; i < e.attributes.length; i++) t.writeMessage(1, j.Value.write, e.attributes[i]);
	...
	j.FeatureResult.write = function (e, t) {
		if (e.objectIdFieldName && t.writeStringField(1, e.objectIdFieldName), e.uniqueIdField && t.writeMessage(2, j.UniqueIdField.write, e.uniqueIdField), e.globalIdFieldName && t.writeStringField(3, e.globalIdFieldName), e.geohashFieldName && t.writeStringField(4, e.geohashFieldName), e.geometryProperties && t.writeMessage(5, j.GeometryProperties.write, e.geometryProperties), e.serverGens && t.writeMessage(6, j.ServerGens.write, e.serverGens), e.geometryType && t.writeVarintField(7, e.geometryType), e.spatialReference && t.writeMessage(8, j.SpatialReference.write, e.spatialReference), e.exceededTransferLimit && t.writeBooleanField(9, e.exceededTransferLimit), e.hasZ && t.writeBooleanField(10, e.hasZ), e.hasM && t.writeBooleanField(11, e.hasM), e.transform && t.writeMessage(12, j.Transform.write, e.transform), e.fields)
			for (var i = 0; i < e.fields.length; i++) t.writeMessage(13, j.Field.write, e.fields[i]);
		if (e.values)
			for (i = 0; i < e.values.length; i++) t.writeMessage(14, j.Value.write, e.values[i]);
		if (e.features)
			for (i = 0; i < e.features.length; i++) t.writeMessage(15, j.Feature.write, e.features[i])
	},
	j.CountResult = {},
	...
	j.QueryResult._readField = function (e, t, i) {
		1 === e ? (t.featureResult = j.FeatureResult.read(i, i.readVarint() + i.pos), t.Results = "featureResult") : 2 === e ? (t.countResult = j.CountResult.read(i, i.readVarint() + i.pos), t.Results = "countResult") : 3 === e && (t.idsResult = j.ObjectIdsResult.read(i, i.readVarint() + i.pos), t.Results = "idsResult")
	},
...
window.ArcGISFeature = class ArcGISFeature {
	constructor(e, t, i, r) {
		if (!e || !t || !i) throw new Error("Source id, map and arcgisOptions must be supplied as the first three arguments.");
		if (!i.url) throw new Error("A url must be supplied as part of the esriServiceOptions object.");
		...
		this.map.addSource(e, Object.assign(s, {
			type: "geojson",
			data: this.getBlankFc()
		}));

		this.getServiceMetadata().then((() => {
			if (!this.supportsPbf) {
				if (!this.supportsGeojson) throw this.map.removeSource(e),
			...
			if (this.esriServiceOptions.useSeviceBounds) {
				...
				e.spatialReference.wkid === 4326 ? this.setBounds([e.xmin, e.ymin, e.xmax, e.ymax]) : this.projectBounds();
			}

			"*" !== this.esriServiceOptions.outFields && (this.esriServiceOptions.outFields = `${this.esriServiceOptions.outFields},${this.serviceMetadata.uniqueIdField.name}`),
				this.enableRequests(),
	...
	setWhere(e) {
		this.esriServiceOptions.where = e;
		this.clearAndRefreshTiles();
	}

	clearWhere() {
		this.esriServiceOptions.where = "1=1";
		this.clearAndRefreshTiles();
	}
	...
	setToken(e) {
		this.esriServiceOptions.token = e;
		this.clearAndRefreshTiles();
	}

	createOrGetTileIndex(e) {
		const t = this.tileIndices.get(e);
		if (t) return t;
		const i = new Map;

		return this.tileIndices.set(e, i),
			i
	}

	createOrGetFeatureCollection(e) {
		const t = this.featureCollections.get(e);
		if (t) return t;
		const i = this.getBlankFc();

		return this.featureCollections.set(e, i),
			i
	}

	createOrGetFeatureIdIndex(e) {
		const t = this.featureIndices.get(e);
		if (t) return t;
		const i = new Map;
		return this.featureIndices.set(e, i),
			i
	}
	...
	async findAndMapData() {
		...
		const t = this.map.getBounds().toArray(),
			i = c.bboxToTile([t[0][0], t[0][1], t[1][0], t[1][1]]);

		if (this.esriServiceOptions.useSeviceBounds && this.maxExtent[0] !== -1 / 0 && !this.doesTileOverlapBbox(this.maxExtent, t)) return;

		const r = this.esriServiceOptions.useStaticZoomLevel ? this.esriServiceOptions.minZoom : 2 * Math.floor(e / 2),
			s = this.createOrGetTileIndex(r),
			n = this.createOrGetFeatureIdIndex(r),
			a = this.createOrGetFeatureCollection(r),
			o = [];
		...
		if (i[2] < r) {
			...
			for (let i = 0; i < e.length; i++) this.doesTileOverlapBbox(e[i], t) && o.push(e[i])
		} else {
			o.push(i);
		}
		...
		for (let e = 0; e < o.length; e++) {
			...
			s.has(t) ? (o.splice(e, 1), e--) : s.set(t, !0)
		}

		if (0 === o.length) return void this.updateFcOnMap(a);

		const l = Math.abs(t[1][0] - t[0][0]) / this.map.getCanvas().width * this.esriServiceOptions.simplifyFactor;

		await this.loadTiles(o, l, n, a),
			this.updateFcOnMap(a);
	}

	async loadTiles(e, t, i, r) {
		return new Promise((s => {
			const n = e.map((e => this.getTile(e, t)));

			Promise.all(n).then((e => {
				e.forEach((e => {
					e && this.iterateItems(e, i, r)
				})),
					s()
			}));
		}));
	...
	iterateItems(e, t, i) {
		e.features.forEach((e => t.has(e.id) || (i.features.push(e), t.set(e.id))));
	}
	...
	get time() {
		...
		return e instanceof Date && (e = e.valueOf()),
			t instanceof Date && (t = t.valueOf()), `${e},${t}`;
	}
	...
	getTile(e, t) {
		...
		return this.time && s.append("time", this.time),
			this.appendTokenIfExists(s),
			new Promise((e => {
				fetch(`${this.esriServiceOptions.url}/query?${s.toString()}`, this.esriServiceOptions.fetchOptions).then((e => "pbf" === this.esriServiceOptions.f ? e.arrayBuffer() : e.json())).then((t => {
					let i;
					try {
						i = "pbf" === this.esriServiceOptions.f ? N(new Uint8Array(t)).featureCollection : t
					} catch (e) {
	...
	updateFcOnMap(e) {
		const t = this.map.getSource(this.sourceId);
		t && t.setData(e);
	}
	...
	getServiceMetadata() {
		if (null !== this.serviceMetadata) return Promise.resolve(this.serviceMetadata);
		const e = new URLSearchParams({
			f: "json"
		});

		return this.appendTokenIfExists(e),
			this.requestJson(`${this.esriServiceOptions.url}?${e.toString()}`, this.esriServiceOptions.fetchOptions).then((e => {
	...
	getFeaturesByLonLat(e, t, i) {
		...
		return this.appendTokenIfExists(r),
			new Promise((e => {
				this.requestJson(`${this.esriServiceOptions.url}/query?${r.toString()}`, this.esriServiceOptions.fetchOptions).then((t => e(t)))
			}));
	}
	...
	getFeaturesByObjectIds(e, t) {
		...
		return this.appendTokenIfExists(i),
			new Promise((e => {
				this.requestJson(`${this.esriServiceOptions.url}/query?${i.toString()}`, this.esriServiceOptions.fetchOptions).then((t => e(t)))
			}));
	}
	...
	projectionEndpointIsFallback() {
		return this.esriServiceOptions.projectionEndpoint === this.fallbackProjectionEndpoint;
	}

	appendTokenIfExists(e) {
		const t = this.esriServiceOptions.token;
