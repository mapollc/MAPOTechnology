...
async function getCounties() {
    if (!global.map.getSource('us_counties')) {
        new ArcGISFeature('us_counties', global.map, {
    ...
    if (!global.map.getLayer('us_counties')) {
        global.map.addLayer({
            id: 'us_counties',
            source: 'us_counties',
            type: 'fill',
...
async function init() {
    let ctr_lat = config.settings.map().lat,
        ctr_lon = config.settings.map().lon;
    ...
    global.map = new maplibregl.Map({
        ...
        hash: true,
        maxPitch: 85,
        pitch: (config.settings.map().pitch ? config.settings.map().pitch : 0),
        bearing: (config.settings.map().bearing ? config.settings.map().bearing : 0),
        attributionControl: false
    });

    // load the conversion class asyncronously into a global scope
    const { Convert } = await import('./utils/convert.js');
    ...
    global.map.on('styleimagemissing', () => loadMapIcons());
    ...
    global.map.on('movestart', () => {
        async function clearRadarCache() {
            const { ClickListener } = await import('./utils/listeners.js');
...
document.onreadystatechange = async () => {
    const preload = async () => {
        ...
        const getTopFires = await helper.api(`${ENV.apiURL}events?test=1`, [['limit', 6]], (config.settings.getUser().uid() == 1 ? true : false));
        if (top) global.dataView.topFires = getTopFires.top;
...
window.onload = async () => {
    ...
    upgrade();
    //helper.storage('mapofire.refresh', new Date().getTime());
...
window.addEventListener('change', async (e) => {
    const { ChangeListener } = await import('./utils/listeners.js');
