const specificURL = window.location.origin + '/',
    config = {
        ...
        platform: () => {
            ...
            if (h.search('wildfiremap.org') >= 0) return 'wildfiremap';
            if (h.search('fireweatheravalanche.org') >= 0) return 'fireweatheravalanche';
            return 'mapofire';
        },
        apiKey: () => {
            if (this.platform === 'fireweatheravalanche') return '191eab18c50c8f5653bdeba13f219bed';
            if (this.platform == 'wildfiremap') return '85f58fa255efe0f779e0dfcd62d87e6d';
            return '50e2c43f8f63ff0ed20127ee2487f15e';
        },
        disableClicks: false,
        wildfire: null,
        ...
        modisZoomLevel: 7,
        workers: {
            incident: new Worker(specificURL + 'src/js/incident-' + version + '.js'),
            fwf: null,
            wwas: null
        },
        fog: {
        ...
        osm: {
            ...
            sources: {
                'openstreetmap': {
                    ...
                    tileSize: 256,
                    attribution: this.defaultAttr + '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                }
            },
            layers: [{
                id: 'osm',
        ...
        terrain: {
            ...
            sources: {
                'esri': {
                    ...
                    tileSize: 256,
                    attribution: this.defaultAttr + '&copy; <a href="https://www.arcgis.com">ArcGIS/ESRI</a>'
                }
            },
            layers: [{
                id: 'terrain',
        ...
        caltopo: {
            ...
            sources: {
                'ct': {
                    type: 'raster',
                    tiles: [this.domain + 'assets/images/tiles/2/{z}/{x}/{y}.png'],
                    tileSize: 256,
                    attribution: this.defaultAttr + '&copy; <a href="https://caltopo.com">CalTopo</a>'
                }
            },
            layers: [{
                id: 'caltopo',
        ...
        fs16: {
            ...
            sources: {
                'usfs2016': {
                    type: 'raster',
                    tiles: [this.domain + 'assets/images/tiles/3/{z}/{x}/{y}.png'],
                    tileSize: 256
                }
            },
            layers: [{
...
<div style="margin-top:5em;font-size:12px;text-align:center;color:var(--blue-gray);line-height:1.3">
 ...
 <a class="footer-link" href="${config.specificURL}logout?service=${config.platform()}&next=${encodeURIComponent(window.location.href)}">Logout</a>&nbsp;&middot;&nbsp;
...
class NWS {
    ...
    async readWWA(id) {
        ...
        if (config.workers.wwas == null) {
            config.workers.wwas = new Worker(config.specificURL + 'src/js/wwas-' + version + '.js');
        }
...
class Wildfires {
    ...
    async getTrackedFires() {
        if (settings.user) {
            const g = await api(config.host + 'api/v1/trackFires/list');
        ...
        if (fires != null && fires.features != null) {
            fires.features.forEach((f, n) => {
                statuses.forEach((stats) => {
                    if (f.properties.status.search(stats) >= 0) {
        ...
        const chk = setInterval(() => {
            if (map.isSourceLoaded('ca_fires')) {
                ...
                if (!map.getLayer('ca_fires')) {
                    ...
                    }).on('mouseenter', 'ca_fires', () => {
                        map.getCanvas().style.cursor = 'pointer';
                    }).on('mouseleave', 'ca_fires', () => {
                        map.getCanvas().style.cursor = 'auto';
                    });
                }
                ...
                if (!map.getLayer('ca_fire_title')) {
                    ...
                    }).on('mouseenter', 'ca_fire_title', () => {
                        map.getCanvas().style.cursor = 'pointer';
                    }).on('mouseleave', 'ca_fire_title', () => {
                        map.getCanvas().style.cursor = 'auto';
                    });
                }
            }
    ...
    incident(wfid, click = false) {
        ...
        cacheIncident(wfid).then(getinc => {
            const fname = this.fireName(getinc.fire.properties.fireName, getinc.fire.properties.type, getinc.fire.properties.incID),
            ...
            config.workers.incident.onmessage = (event) => {
                ...
                const donate = `<div class="donate"><div><p>Your contribution makes a difference. Donate to support our work.</p>
                    <a href="${config.donateLink}?utm_campaign=Fundraising&utm_source=mapofire&utm_medium=incident_page" id="map_fire_modal_donate" target="blank" class="btn btn-green" onclick="this.parentElement.parentElement.remove();return true">
    ...
    async caPerimeters(update = false) {
        ...
        if (update && map.getSource('ca_perimeters')) {
            ...
            if (!map.getLayer('ca_perimeters_outline')) {
                ...
                }).on('mouseenter', 'ca_perimeters_fill', () => {
                    map.getCanvas().style.cursor = 'pointer';
                }).on('mouseleave', 'ca_perimeters_fill', () => {
                    map.getCanvas().style.cursor = 'auto';
                });
            }
        }
...
class Layers {
    ...
    async lightning() {
        map.addSource('lightning1', {
            ...
            maxzoom: 15,
            tiles: [
                //config.host + 'api/v1/lightning?key=' + config.apiKey() + '&x={x}&y={y}&z={z}&s=256&t=5'
            ...
            maxzoom: 15,
            tiles: [
                //config.host + 'api/v1/lightning?key=' + config.apiKey() + '&x={x}&y={y}&z={z}&s=256&t=6'
...
class ClickListener {
    ...
    closeDataForm() {
        ...
        if (r.getAttribute('data-active') == '1') {
            r.removeAttribute('data-active');
        }

        if (fw.getAttribute('data-active') == '1') {
    ...
    createLayers() {
        const scrlt = localStorage.getItem('mapofire.impactScroll');
        let laylist = '';
        ...
        Object.entries(layers.categories).forEach((g) => {
            ...
            layers.layers[g[0]].forEach((l, i) => {
                ...
                lay += '<li class="layer" data-p="' + l.perms + '" data-id="' + l.id + '" title="' + l.name + '"><div class="checkbox">' +
                    (hasPerms ? '<input type="checkbox" class="layChkBx" data-action="toggle-layer" data-type="' + l.type + '" id="' + l.id + '"' + (isChecked ? ' checked' : '') + '>' : premFeature) + '</div>' +
                    '<div class="desc"><label for="' + l.id + '">' + l.name + '</label><span>' + l.desc/*layerDesc[g[0]][i]*/ + '</span>';

                lay += this.layerExtras(l, hasPerms, isChecked) + '</div></li>';
            });
    ...
    layerExtras(l, hasPerms, isChecked) {
        let smokeOptions = '',
            lay = '';
        ...
        if (l.id == 'perimeters') {
            lay += '<div id="perimeterSize" data-action="change-perim-size"><i class="fad fa-filters" style="color:#b9b9b9"></i><input type="range" class="slider" min="0" max="1000" step="25" value="' + settings.get('perimeters').minSize + '">' +
                '<div id="pSize" style="width:69.11px">' + settings.get('perimeters').minSize + ' acres</div></div>';
        }

        if (hasPerms) {
            if (l.id == 'ndfd') {
                lay += '<div id="models"><i class="far fa-filter-list" style="color:#b9b9b9"></i><select' + (!isChecked ? ' disabled' : '') + ' id="forecastModel" data-action="ndfd" style="min-width:170px"><option ' + (settings.get('special').forecastModel == 'apparent_temperature' || !settings.get('special') ? 'selected ' : '') + 'value="apparent_temperature">Temperature</option>' +
                    ...
                    '<select' + (!isChecked ? ' disabled' : '') + ' id="fcstTime" data-action="ndfd" data-type="reg" style="min-width:100px;max-width:35%">' + initNDFDTimes() + '</select></div>';
            } else if (l.id == 'sfp') {
                lay += '<div id="sfpDate"><i class="far fa-filter-list" style="color:#b9b9b9"></i><select' + (!isChecked ? ' disabled' : '') + ' id="sfpDateSelect" data-action="sfp-date">' + sfpTimes() + '</select></div>';
            } else if (l.id == 'spc') {
                lay += '<div id="otlks"><i class="far fa-filter-list" style="color:#b9b9b9"></i><select' + (!isChecked ? ' disabled' : '') + ' id="otlkType" data-action="spc-outlook" style="min-width:170px">' +
                    ...
                    '<select' + (!isChecked ? ' disabled' : '') + ' id="otlkDay" data-action="spc-outlook" style="min-width:100px"><option ' + (settings.special().otlkDay() == 1 ? 'selected ' : '') + 'value="1">Day 1</option>' +
                ...
                lay += '<div id="ercs"><i class="far fa-filter-list" style="color:#b9b9b9"></i><select' + (!isChecked ? ' disabled' : '') + ' id="erc_time" data-action="erc_time" style="min-width:197px">' +
                ...
                lay += '<div id="viSmokes"><i class="far fa-filter-list" style="color:#b9b9b9"></i><select' + (!isChecked ? ' disabled' : '') + ' id="vi_smoke_time" data-action="vi_smoke_time" style="min-width:160px">' +
                    smokeOptions + '</select></div>';
            } else if (l.id == 'sfcSmoke') {
                lay += '<div id="sfcSmokes"><i class="far fa-filter-list" style="color:#b9b9b9"></i><select' + (!isChecked ? ' disabled' : '') + ' id="sfc_smoke_time" data-action="sfc_smoke_time" style="min-width:160px">' +
                    smokeOptions + '</select></div>';
            }
        }
    ...
    basemaps() {
        ...
        tileNames.forEach((n, c) => {
            ...
            l += '<li><div class="radio">' + (hasPerms ? '<input type="radio" class="basemap-option" data-action="change-basemap" name="bsmo" data-tile="' + n + '"' + (n == settings.getBasemap() ? ' checked' : '') + '>' : premFeature) +
    ...
    account() {
        if (settings.user == null) {
            window.location.href = config.domain + 'secure/login?service=' + config.platform() + '&next=' + encodeURIComponent(window.location.href);
        } else {
            let ms = '';
            const userProfile = `<div id="sync"><i class="fa-regular fa-arrow-down-to-line" aria-hidden="true"></i><span>Account last synced ${timeAgo(settings.user.settings.synced)}</span></div>
                <div class="btn-group centered"><a target="blank" class="btn btn-gray" style="width:100%" href="${config.domain}account/settings?ref=mapofire">Manage account</a></div>`;
            ...
            if (!settings.subscriptions().valid()) {
                ms = ''/*'<p style="color:#555;font-size:14px">You don\'t have a subscription to Map of Fire Premium. <a href="' + config.domain + 'checkout?price_id=' + config.sub_id + '&next=' + encodeURIComponent(window.location.href) + '">Try it now!</a>'*/;
            } else {
                ms = '<div style="display:inline-flex;width:100%;justify-content:space-between;gap:1em"><div style="display:inline-flex;flex-direction:column;gap:0.45em">' +
                    ...
                    '<a href="' + config.domain + 'account/billing/portal?sid=' + settings.subscriptions().subID() + '&cid=' + settings.subscriptions().customerID() + '&next=' + encodeURIComponent(window.location.href) + '">Manage</a></div>';
            }
                    ...
                    tracked.forEach((id) => {
                        ...
                        if (fire != null) {
                            const p = fire.properties,
                                name = config.wildfire.fireName(p.name, p.type, p.incidentId),
                            ...
                            content += `<li id="my-fire-incident" data-action="my-fire-inc" data-coords="${JSON.stringify(fire.geometry.coordinates)}">
                            <div class="header"><h3 style="line-height:1.2;color:#537376">${name}</h3>
                            <i class="fas fa-star" data-action="my-fire-unfollow" title="Unfollow this incident" data-name="${name}" data-wfid="${p.wfid}" style="color:var(--yellow);cursor:pointer"></i></div>
...
function toggleLayer(e) {
    ...
    if (layer == 'newFires') {
        ...
        map.setLayoutProperty('all_fires_layer', 'visibility', (checked ? 'visible' : 'none'))
            ...
            .setLayoutProperty('ca_fires', 'visibility', (checked ? 'visible' : 'none'))
            .setLayoutProperty('ca_fire_title', 'visibility', (checked ? 'visible' : 'none'));
        ...
        map.setLayoutProperty('perimeters_outline', 'visibility', (checked ? 'visible' : 'none'))
            ...
            .setLayoutProperty('ca_perimeters_outline', 'visibility', (checked ? 'visible' : 'none'))
            .setLayoutProperty('ca_perimeters_fill', 'visibility', (checked ? 'visible' : 'none'))
            .setLayoutProperty('ca_perimeters_title', 'visibility', (checked ? 'visible' : 'none'));
        ...
        if (map.getSource('ev')) {
            map.setLayoutProperty('ev', 'visibility', (checked ? 'visible' : 'none'));
        } else {
            if (checked) {
                new Layers().ev();
            }
...
function marketing() {
    const m = localStorage.getItem('mapofire.marketing'),
        REPEAT_EVERY_DAY = 1.5;
    ...
    if (m == null || (Number(Date.now() - Number(m)) > 86400000 * REPEAT_EVERY_DAY)) {
        const content = `<p style="margin:1.5em 0;font-size:18px;font-weight:400">Map of Fire delivers life-saving wildfire and public safety information to everyone, free of charge. Your contribution directly fuels our ability to continue this critical work. Thank you for your support!</p>
            ...
            <input type="button" id="donate_cta" class="btn btn-red dn" value="Yes! I\'ll help out" data-action="start-donation">
            <input type="button" id="donate_dismiss" class="btn btn-gray dn" value="Later, maybe" data-action="close-data-form"></div>`;
        ...
        localStorage.setItem('mapofire.marketing', config.curTime.getTime());
    }
}

function upgrade() {
    const items = ['dark_mode', 'dispatch', 'dispatch_time', 'impactScroll', 'marketing', 'version', 'clicks', 'tracked'];
    ...
    async do() {
        ...
        if (this.query.length > 0) {
            ...
            if (crds != null) {
                ...
                /* query database of cities and GIS features */
                if (this.query.length >= 3) {
                    count += await this.apiSearch();
                }
            }
        }
        ...
        if (count == 0) {
            if (this.query == '') {
                ...
                this.standby.style.userSelect = 'none';
                this.standby.style.cursor = 'auto';

                if (this.standby.querySelector('i')) {
...
function init() {
    ...
    map = new mapboxgl.Map({
        ...
        if (document.querySelector('li#fwf').getAttribute('data-active') == 1) {
            config.disableClicks = true;
            ...
            if (config.workers.fwf == null) {
                config.workers.fwf = new Worker(config.specificURL + 'src/js/fwf-' + version + '.js');
            }
            ...
            config.workers.fwf.postMessage({
                ...
                lon: e.lngLat.lng,
                units: {
                    temp: settings.weather().temp(),
                    wind: settings.weather().wind()
                }
            });
        ...
        icons.forEach((icon) => {
            ...
            if (!map.hasImage('fire-icon' + icon)) {
                map.loadImage(config.domain + 'assets/images/icons/fire/fire-icon' + icon + '.png', (error, image) => {
        ...
        ['helicopter', 'plane_tactical', 'plane_large', 'plane_small'].forEach((icon) => {
            if (!map.hasImage(icon)) {
                map.loadImage(config.domain + 'assets/images/icons/fire/' + icon + '.png', (error, image) => {
                    if (error) throw error;
        ...
        /* add modis icons */
        for (let i = 1; i < 4; i++) {
            map.loadImage(config.domain + 'assets/images/icons/fire/modis' + i + '.png', (error, image) => { if (error) throw error; map.addImage('modis' + i, image); });
        }
...
async function preload() {
    ...
    if (token != null) {
        const get = await api(config.apiURL + 'user/get/mapofire', [['token', token[1]]]);
        usr = get.user;
        ...
        if (usr != null) {
            ...
            a.setAttribute('title', 'Account Settings');
        }
    } else {
    ...
    document.querySelectorAll('nav ul li').forEach((l) => {
        const ttip = JSON.parse(l.dataset.tooltip).text;

        l.removeAttribute('data-tooltip');
        l.setAttribute('title', ttip);
    });
...
function complete() {
    ...
    q.addEventListener('focus', () => {
        if (!trending && document.querySelectorAll('#search-results li.trending').length == 0) {
            addTrending();
        }
    });
}
...
function addTrending() {
    ...
    if (sr.querySelector('li.standby').style.display == 'none') {
        sr.querySelector('li.standby').style.display = 'inline-flex';
    }
    ...
    if (topFires != null && topFires.length > 0) {
        ...
        for (let i = 0; i < topFires.length; i++) {
            ...
            if (fire) {
                const fireName = config.wildfire.fireName(fire.properties.name, fire.properties.type, fire.properties.incidentId),
                    acres = conversion.acres(fire.properties.acres),
                    who = Math.round(parseFloat(topFires[i].count / clicks) * 100) + '% of our users are looking at the ' + fireName,
                ...
                li.setAttribute('data-type', 'incident');
                li.setAttribute('data-wfid', p.wfid);
                li.setAttribute('title', topFires[i].trending ? who : fireName);
...
window.onload = async () => {
    ...
    const top = await api(config.apiURL + 'events', [['limit', 6]]);
    topFires = top.top;
    ...
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register(config.specificURL + 'src/js/service-worker-' + version + '.js')
            .then(registration => {
...
window.addEventListener('click', async (e) => {
    const target = e.target,
        sr = document.querySelector('#search-results'),
        actionsThatOpenImpact = ['account', 'basemap', 'layers', 'legend', 'myfires'],
        actionElement = target.closest('[data-action]'),
        action = actionElement ? actionElement.dataset.action : null,
        clickListener = new ClickListener(target, sr),
        actionHandlers = {
            ...
            'start-donation': () => {
                window.open(config.donateLink + '?utm_campaign=Fundraising&utm_source=mapofire&utm_medium=popup');
            ...
            'layers': () => clickListener.createLayers(),
    ...
    if (e.target.id == 'q') {
        if (!e.ctrlKey && !e.altKey && e.key != 'Enter' && e.key != 'Shift') {
            const results = document.querySelector('#search-results'),
                standby = results.querySelector('.standby');
