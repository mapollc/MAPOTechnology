...
$cacheKey = $host.$_SERVER['REQUEST_URI'];
$memcached = new Memcached();
$memcached->addServer('localhost', 11211);
$cache = $memcached->get($cacheKey);
$cacheTime = $memcached->get($cacheKey.'-time');

if (!$cache || filemtime('/home/mapo/public_html/mapofire.com/v'.$version.'/mf-incident.php') > $cacheTime || filemtime('/home/mapo/public_html/mapofire.com/incident.php') > $cacheTime) {
    ob_start();
    ...
    if (!$fire->error) {
        ...
        $contain = $fire->properties->containment;
...
<html lang="en-US" xmlns:og="http://ogp.me/ns#">
    <head>
        ...
        <link rel="stylesheet" href="https://www.mapofire.com/src/css/inc.app-<?=$version?>.css">
        ...
        <link rel="shortlink" href="https://mapofire.com/i/<?=$_GET['wfid']?>" />
    </head>
    <body>
        ...
        <header>
            <div class="container">
                ...
                <nav>
                    ...
                    <ul>
                        <li><a href="https://<?=$host?>/">Back to Map</a></li>
                        <li><a href="https://<?=$host?>/list-of-wildfires">List of Fires</a></li>
                        <li><a id="userlink" href="<?=$loginLink?>">Login</a></li>
                    </ul>
                </nav>
            </div>
        </header>
            ...
            <div class="container">
                ...
                <section>
                    <?if($fire->time->year < date('Y')) {
                        echo '<div class="banner">This is a historical fire from '.$fire->time->year.' and it is not currently active.</div>';
                    }?>
                    ...
                    <div class="stats">
                        ...
                        <div class="stat">
                            ...
                            <div class="v" title="<?=$contain == 'N/A' ? 'Unknown if this incident is contained' : 'This incident is ${contain} contained'?>">
                                ...
                                <?if($contain != 'N/A'){?>
                                    <div class="line">
                                        <div style="width:<?=$contain . ($contain == '100%' ? ';opacity:1' : '')?>"></div>
                                    </div>
                                <?}?>
                                </div>
                            </div>
                        ...
                        <div class="stat">
                            <a href="#" id="follow" class="btn btn-sm btn-yellow" onclick="return false" data-mode="follow" data-id="<?=$fire->properties->wfid?>" style="margin:0"><i class="far fa-plus"></i>Follow this incident</a>
                        </div>
                    </div>
                ...
                <section>
                    ...
                    <div class="donate">
                        <div>
                            <p>Your contribution makes a difference. Donate to support our work.</p>
                            <a href="https://mapofire.com/donate?utm_campaign=Fundraising&utm_source=mapofire&utm_medium=fire_page" id="incident_fire_modal_donate" target="blank" class="btn btn-green" onclick="this.parentElement.parentElement.remove();return true"><i class="fas fa-heart"></i>Contribute</a>
                        </div>
                        <i class="far fa-xmark" onclick="this.parentElement.remove();return false"></i>
                    </div>
                    ...
                    <div class="row inciweb">
                        <div class="col w100">
                            ...
                            <div class="text">
                                <?if($photo) {
                                    echo '<div class="row"><div class="col w25"><figure><img src="'.$photo.'&small=1">'.
        ...
        <footer>
            <div class="container">
                <div class="row space-between align-center">
                    ...
                    <div class="col w50">
                        <ul>
                            <li><a target="blank" href="https://donate.stripe.com/3csg1F7PF8gpfni003?__prefilled_amount=2500&utm_campaign=Fundraising&utm_source=mapofire&utm_medium=fire_page_footer">Support</a></li>
        ...
        <script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js', new Date());gtag('config', '<?=$ga_id?>');(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','GTM-NZXGFH6');const incident=<?unset($fireJSON->fire->inciweb);unset($fireJSON->metadata);$fireJSON->fire->geometry->near_long = $near_long;echo json_encode($fireJSON->fire)?>;</script>
        <script src="https://www.mapofire.com/src/js/inc.app-<?=$version?>.js"></script>
    </body>
</html>
<?
    $htmlContent = ob_get_clean();
    $memcached->set($cacheKey, $htmlContent, 900);
    $memcached->set($cacheKey.'-time', time(), 900);
