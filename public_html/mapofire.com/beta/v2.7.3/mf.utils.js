...
export class Convert {
    ...
    async utm(a, b) {
        return new (await loadUtils()).UTM().toUTM(Number(a), Number(b));
    }
...
export class Evacuations {
    ...
    displayEvacs(data) {
        ...
        map.moveLayer('evac', 'perimeters_fill');
    }
}
...
export class Wildfires {
    ...
    fireStats(history, incID = null) {
        ...
        if (onlyNegative) statSentence.push(`This fire was discovered at ${conversion.sizeFormat(last.acres)}.`);

        if (diff > 0) {
            const isCurrentYear = year === config.curTime.getFullYear();
            statSentence.push(`Over ${duration}${!isCurrentYear ? ` in ${year}` : ''}, this fire grew ${isCurrentYear ? 'by' : 'to'} ${overall}, averaging ${conversion.sizeFormat(avgValue)} of growth per ${growthUnit} (when active).`);
        } else {
            statSentence.push(`Incident reporting has decreased this fire in size by ${Math.abs(diff).toFixed(2).replace(/\.?0+$/, '')} acres.`);
        }

        if (maxGrowth > 0) statSentence.push(`The largest single growth was ${conversion.sizeFormat(maxGrowth)} ${whenGrew}.`);
    ...
    processFires(fires, type, update) {
        ...
        fires.features.forEach(f => {
            ...
            if (type === 'new' && p.acres >= 100 && !dataView.newFires.some(item => item.properties.incidentId === p.incidentId)) {
                dataView.newFires.push(f);
            }
        });
    ...
    async cacheIncident(wfid) {
        ...
        try {
            ...
            if (settings.fire().cache() && !debugMode) {
...
export class ClickListener {
    ...
    async account() {
        ...
        if (!settings.subscriptions().valid()) {
            if (settings.getUser().role() == config.PERMISSION_LEVELS.LICENSEE) {
    ...
    archive() {
        if (settings.hasPermissions(config.PERMISSION_LEVELS.PREMIUM)) {
            ...
            new Popup('Historical Wildfires').create(null, `<p style="font-size:14px;line-height:1.2">See historical wildfires by selecting a year in our archive.</p>
...
export function marketing(override = false, utm = null) {
    ...
    if (!override && settings.getUser().role() == config.PERMISSION_LEVELS.ADMIN && settings.getUser().role() == config.PERMISSION_LEVELS.LICENSEE) shouldShow = false;
...
export function addTrending() {
    ...
    if (dataView.topFires?.length) {
        dataView.topFires.forEach(i => {
            ...
            const acres = conversion.sizeFormat(fire.properties.acres);
...
export async function startReportProcess(e) {
    ...
    api(`${ENV.apiURL}geocode/incident${(data != null ? '/near' : '')}`, [['lat', e.lngLat.lat], ['lon', e.lngLat.lng]], true)
        .then(res => {
            if (data == null) {
...
export function contextMenu(e, isTouch = false) {
    ...
    const div = document.createElement('div'),
        ...
        options = [
            ...
            { text: 'Report new fire', task: async () => (await loadUtils()).startReportProcess(e), hasPerms: true }
        ];
