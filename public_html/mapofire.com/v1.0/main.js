...
function setMapType() {
    ...
    $('#tileChange ul.select-options li').each(function () {
        if ($(this).attr('rel') == settings.tile) {
            $('#tileChange .select-styled').html($(this).text());
        }
    });
}
        ...
        if (c === true) {
            ...
            if (a == 'viSmoke' || a == 'sfcSmoke') {
                if ($('#layerLabel').length == 0) {
                    $('body').prepend('<div id="layerLabel"><b style="font-weight:400">Smoke Fcst Time:</b> ' + dateTime(new Date(hrrrSmokeTime.fcst + 'Z').getTime() / 1000, 1) + '</div>');
                }
            }
...
function saveSession(h) {
    ...
    var set = {
        ...
        fireDisplay: fd.find('option:selected').val()
    };
...
function wildfire(fire) {
    ...
    var a = '<div class="row even" style="align-items:center"><div class="col" style="flex-direction:column"><div style="display:flex;align-items:center">' + edit + '<h1>' + n + '</h1></div><span class="coords">' + coords(fire.geometry.lat, fire.geometry.lon) + '</span></div>' +
        ...
        b = '<div class="row cta"><div class="col"><div class="title">status</div><div class="desc"><span class="status ' + st + '">' + st.toUpperCase() + '</span></div></div>' +
            ...
            '<div class="col"><div class="title">containment</div><div class="desc">' + fire.properties.containment + '</div></div></div>' +
    ...
    if (fire.inciweb) {
        ...
        if (fire.inciweb.photo) {
            $('<div class="photo"><a target="blank" href="https://www.mapofire.com/src/images/incident?path=' + fire.inciweb.photo.url + '"><img src="https://www.mapofire.com/src/images/incident?path=' + fire.inciweb.photo.url + '" title="' + fire.inciweb.photo.caption + '"></a></div>').insertAfter('#modal #e');
        }
...
$(document).ready(function () {
    ...
    $(document).on('change', '[class=layersMenu] input[type=checkbox]', function () {
        ...
        if (map.hasLayer(viSmoke) === false && map.hasLayer(sfcSmoke) === false) {
            $('#layerLabel').remove();
        }
