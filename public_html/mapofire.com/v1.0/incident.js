            ...
            if (r == null || r.features.length == 0) {
                ...
                for (var i = 0; i < r.features.length; i++) {
                    ...
                    if (aq != null) {
                        ...
                        $('#airq').html('<div class="airQ" data-id="' + q.AQSID + '" title="' + l + ': ' + hm + '" style="background-color:' + c + (aq >= 150 ? ';color:#fff' : '') + '">' + l + '</div>');
...
function alerts(a, b) {
    $.ajax({
        url: 'https://api.weather.gov/points/' + a + ',' + b,
        method: 'GET',
        dataType: 'json',
        success: function (data) {
            var zone = data.properties.fireWeatherZone.match(/([A-Z]{2})Z([0-9]{3})/)[0];

            $.ajax({
                url: 'https://api.weather.gov/alerts/active/zone/' + zone,
                method: 'GET',
                dataType: 'json',
                success: function (wwa) {
                    var re = '';

                    if (wwa.features.length == 0) {
                        /* no wwas */
                    } else {
                        wwa.features.forEach(function (a) {
...
$(document).ready(function () {
    ...
    if (settings.tile) {
        switch (settings.tile) {
            ...
            case 'delorme': tile = delorme; break;
...
$(document).on('click', '[class^=airQ]', function () {
    window.open('https://tools.airfire.org/monitor-data-report/v4/report?databaseversion=4.0&webserviceapi=4.0&monitors=' + $(this).attr('data-id') +'_01');
});