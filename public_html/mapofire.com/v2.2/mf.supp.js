...
class Popup {
    constructor(title, tall = false) {
        this.header = '<div class="header"' + (!title ? ' style="margin-bottom:0"' : '') + '><h1>' + title + '</h1><span id="close-popup" title="Close popup" class="far fa-xmark-large"></span></div>';
        this.tall = tall;
        this.dialog = null;
    }
                ...
                if (!map.getLayer('stns')) {
                    map.addLayer({
                        ...
                        paint: {
                            'text-color': [
                                'case',
                                ['all', ['>=', ['get', 'temp'], -20], ['<', ['get', 'temp'], -10]], '#000',
                                ['all', ['>=', ['get', 'temp'], -10], ['<', ['get', 'temp'], 0]], '#fff',
                                ['all', ['>=', ['get', 'temp'], 0], ['<', ['get', 'temp'], 10]], '#fff',
                                ['all', ['>=', ['get', 'temp'], 10], ['<', ['get', 'temp'], 20]], '#000',
                                ['all', ['>=', ['get', 'temp'], 20], ['<', ['get', 'temp'], 30]], '#000',
                                ['all', ['>=', ['get', 'temp'], 30], ['<', ['get', 'temp'], 40]], '#fff',
                                ['all', ['>=', ['get', 'temp'], 40], ['<', ['get', 'temp'], 50]], '#fff',
                                ['all', ['>=', ['get', 'temp'], 50], ['<', ['get', 'temp'], 60]], '#000',
                                ['all', ['>=', ['get', 'temp'], 60], ['<', ['get', 'temp'], 70]], '#000',
                                ['all', ['>=', ['get', 'temp'], 70], ['<', ['get', 'temp'], 80]], '#000',
                                ['all', ['>=', ['get', 'temp'], 80], ['<', ['get', 'temp'], 90]], '#000',
                                ['all', ['>=', ['get', 'temp'], 90], ['<', ['get', 'temp'], 100]], '#000',
                                ['all', ['>=', ['get', 'temp'], 100], ['<', ['get', 'temp'], 110]], '#000',
                                ['all', ['>=', ['get', 'temp'], 110], ['<', ['get', 'temp'], 120]], '#000',
                                '#000'
                            ]
                        },
                        layout: {
...
function dateTime(it, time = false, timezone = false) {
    ...
    return months[t.getMonth()] + ' ' + t.getDate() + ',&nbsp;' + t.getFullYear() + (time ? ' at ' + a : '') + (timezone ? ' ' + tz : '');
}
...
function setHeaders(a, b, c) {
    const title = `${a} | ${productName}`,
        s = window.location.search,
        h = window.location.hash;

    window.history.pushState({
        "pageTitle": title
    }, '', urlSpecific + b.replace('incident/', '').replace('wildfire/', 'fires/') + (s ? s : '') + (h ? h : ''));
...
async function saveSession(method = true, msg) {
    if (navigator.onLine) {
        ...
        const data = await api(host + 'api/v1/session', send);
...
async function onRasterLayerClick(coords) {
    ...
    if (rth && rth.layout.visibility.toString() == 'visible' && map.getZoom() >= 6) {
        const popup = new Popup('Wildfire Risk', true).create('<div id="spinner" class="sm" style="display:block;text-align:center;margin:0 auto"></div>'),
            ...
            getRisk = async (fips) => {
                const data = await api(apiURL + 'risk/' + fips);
        ...
        for (let i = 0; i < features.length; i++) {
            ...
            if (fire_layers.includes(feature.source)) {
                ...
                if (settings.fire().display()) {
                    ...
                    window.open(host + feature.properties.url);
                }
            ...
            if (feature.source == 'outlook') {
                const color = (feature.properties.fill == '#66A366' || feature.properties.fill == '#ff3333' ? ';color:#fff' : ''),
                    ...
                    <a href="#" id="readSPC" data-type="${settings.special().otlkType()}" data-day="${settings.special().otlkDay()}" onclick="return false" style="display:block;text-align:center">Read the forecast</a>`;
            ...
            if (modis_layers.includes(feature.source)) {
                ...
                <div class="item"><div class="t">Satellite</div><div class="v">VIIRS/${p.satellite}</div></div>
            ...
            if (feature.source == 'erc') {
                ...
                if (settings.special().erc() == null || settings.special().erc() == 'obs') {
                    ...
                    date = dateTime((new Date().getTime()) + 86400) + ' (Tomorrow)';
                ...
                new Popup('Energy Release Component').create(`<div class="item"><div class="t">ERC Date</div><div class="v">${date}</div></div>
                    <div class="item"><div class="t">Area (PSA)</div><div class="v">${psa}</div></div>
                    <div class="item"><div class="t">PSA Code</div><div class="v">${code}</div></div>
                    ${erc}
            ...
            if (feature.source == 'stns') {
                ...
                new Popup('Current Weather').create('<div class="item"><div class="t">Station Name</div><div class="v">' + p.NAME + '</div></div>' +
            ...
            if (feature.source == 'evac') {
                ...
                new Popup('Evacuations').create('<div class="item"><div class="t">Level</div><div class="v">Level ' + p.level + '</div></div>' +
                    ...
                    '<div class="item"><div class="t">County</div><div class="v">' + p.county + ' County, ' + p.state + '</div></div>' +
...
window.addEventListener('change', (e) => {
    ...
    /* toggle dark mode */
    if (target.id == 'dark_mode') {
        let useTile = '',
            url = `${host}src/css/mf.app-dark_mode-${version}.css`,
    ...
    /* change erc date */
    if (document.querySelector('#erc') && document.querySelector('#erc').checked && (target.id == 'erc_time')) {
        const s = document.querySelector('#erc_time');
