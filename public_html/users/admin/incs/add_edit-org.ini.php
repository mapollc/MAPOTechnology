...
?>
    ...
    <form action="" method="post">
        ...
        <input type="text" class="input" style="max-width:200px" name="org_key" value="<?= $repopulate ? $_POST['org_key'] : $row['org_key'] ?>" <?= $function == 'edit' ? ' disabled' : '' ?>>

        <label>Organization Full Name</label>
        <input type="text" class="input" name="name" value="<?= $repopulate ? $_POST['name'] : $row['name'] ?>">

        <label>Administrator Email</label>
        <input type="email" class="input" name="admin_email" value="<?= $repopulate ? $_POST['admin_email'] : $row['admin_email'] ?>">

        <label>Max Users</label>
        <input type="number" class="input" style="width:70px" name="max_users" value="<?= $repopulate ? $_POST['max_users'] : 5 ?>" placeholder="5" min="5" max="100">

        <label>Start Period</label>
        <input type="datetime-local" class="input" name="start_period" style="max-width:195px" step="3600" min="<?= date('Y-m-d\TH:00') ?>" max="<?= date('Y-m-d\TH:00', strtotime('+1 month')) ?>" value="<?= $repopulate ? $_POST['start_date'] : date('Y-m-d\TH:00') ?>" <?= $row['active'] && $function == 'edit' ? ' readonly' : '' ?>>

        <label>End Period</label>
        <input type="datetime-local" class="input" name="end_period" style="max-width:195px" value="<?= $repopulate ? $_POST['end_date'] : date('Y-m-d\TH:00', ($function == 'edit' ? $row['end_period'] : strtotime('+1 month'))) ?>" <?= $function == 'create' ? ' readonly' : '' ?>>
