...
namespace {
	...
	function parseUserAgent( $u_agent = null ) {
		return \donatj\UserAgent\parseUserAgent($u_agent);
	}
}
...
REGEX
        ...
        switch ($browser) {
            case 'Edge': $browser = 'Mircosoft '.$browser; break;
            case 'Safari': $browser = 'Apple '.$browser; break;
            case 'Chrome': $browser = 'Google '.$browser; break;
            default: $browser;
        }

        if ($platform == 'Android') {
            preg_match('/Android\s([0-9]+)/', $u_agent, $m);
            $platform = $m[0];
        }

        if ($platform == 'Windows') {
            preg_match('/Windows\sNT\s([0-9.]+)/', $u_agent, $m);
            $platform = 'Windows '.$m[1];
        }
