...
function outlookText($spc)
{
    ...
    $text = preg_replace('/^<p>(.*?)<\/p>/', '<p style="padding-bottom:.75em">$1</p>', $text);
...
if ($function == 'text') {
    ...
    if ($method == 'fire' && $_REQUEST['day'] > 2 || $method == 'severe' && $_REQUEST['day'] > 4) {
        ...
        if ($method == 'fire') {
            ...
            if ($_REQUEST['day'] == 4) {
                $image[] = 'exper/day4-8/day4prob.png';
            } else {
    ...
    if ($method == 'fire') {
        $url = [
            'https://www.spc.noaa.gov/products/fire_wx/day' . $_REQUEST['day'] . 'fw_dryt.lyr.geojson',
    ...
    for ($x = 0; $x < count($url); $x++) {
        ...
        for ($i = 0; $i < count($json->features); $i++) {
            ...
            if ($c) {
                $otlk[] = [
                    'type' => 'Feature',
                    'geometry' => ['type' => $json->features[$i]->geometry->type, 'coordinates' => $c],
                    'properties' => ['name' => $label, 'stroke' => $stroke, 'fill' => $fill, 'issue' => $iss, 'valid' => $v, 'expires' => $e]
                ];
            }
        }
    }

    $returnJson = ['type' => 'FeatureCollection', 'features' => $otlk];
}