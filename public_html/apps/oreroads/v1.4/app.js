...
class Modal {
    ...
    roadReport(name, int = 0) {
        if (this.json.length == 0) {
            ...
            if (this.json.length > 1) {
                ...
                get('#modal .updated').insertAdjacentHTML('beforebegin', '<select id="changeRW" style="width:100%" data-json=\'' + JSON.stringify(this.json) + '\'>' + ops + '</select>');
            }
    ...
    rwis(cams = []) {
        ...
        <div style="display:inline-flex;flex-direction:column;justify-content:center;gap:1em;flex:1 1 50%">
            <div style="text-align:center"><h5${surface.pavement[0] < 32 ? ' style="color:#D33737"' : ''}>${pvt}</h5><span class="cl">pavement temp</span></div>
        ...
        if (weather.wind.dir != 'null') {
            content += `<div style="display:flex;gap:1em">
                <div class="card vert">
                    <h5>${weather.wind.speed == 'null' ? 'N/A' : Math.round(weather.wind.speed) + ' mph'}</h5>
                    <span class="cl">wind speed</span>
                    <h5>${weather.wind.gust == 'null' ? 'N/A' : Math.round(weather.wind.gust) + ' mph'}</h5>
        ...
        get('#modal .popup-content').innerHTML = content;

        if ((new Date().getTime() / 1000) - this.json.updated > (60 * 60 * 24 * 2)) {
            get('#modal .popup-content').insertAdjacentHTML('beforebegin', '<p class="chreq">Station is likely offline and data is expired</p>');
        }
    ...
    incident() {
        ...
        get('#modal .updated').style.textAlign = 'left';
        get('#modal .updated').setAttribute('title', 'Last updated ' + new Date(data.updated * 1000).toLocaleString().split(', ').join(' at '));
    ...
    webcam(geo = null) {
        ...
        this.json.forEach((camera, i) => {
            ...
            get('#modal .rows').insertAdjacentHTML('afterend', `<h2 class="wc"${this.json.length > 0 && i != this.json.length - 1 ? ' style="margin-top:1em"' : ''}>
...
class Data {
    ...
    displayWWAs(json) {
        ...
        if (!map.getLayer('wwas_fill')) {
            map.addLayer({
                ...
                layout: {
                    visibility: this.defaultLayer('wwas')
                }
            }, 'roads_path').addLayer({
                id: 'wwas_text',
                type: 'symbol',
                ...
                layout: {
                    'symbol-placement': 'point',
                    'symbol-spacing': 400,
                    'text-font': ['Source Sans Pro Medium'],
                    'text-field': ['concat', ['get', 'phenomenon'], ' ', ['get', 'significance']],
                    'text-justify': 'center',
                    'text-size': 14,
                    'text-allow-overlap': false,
                    visibility: this.defaultLayer('wwas')
                ...
                map.getCanvas().style.cursor = 'auto';
            }).on('click', 'wwas_fill', async (e) => {
                modal.innerHTML = template2;
                get('#modal h1').innerHTML = 'Weather Alerts';
                get('#modal .popup-content').innerHTML = `<span id="close"></span>${loading}`;
                modal.style.display = 'flex';

                let alerts = '<p>There are no weather alerts for this area currently.',
                    resp = await api(apiURL + 'nws', [['lat', e.lngLat.lat], ['lon', e.lngLat.lng]]);

                if (resp) {
                    if (resp.alerts && resp.alerts.length > 0) {
                        alerts = '<ul>';

                        resp.alerts.forEach((wx) => {
                            alerts += `<li>${wx.headline.replace(wx.event, `<a href="https://alerts.weather.gov/id/${wx.id}" target="blank">${wx.event}</a>`)}</li>`;
                        });

                        alerts += '</ul>';

                        get('#modal h1').innerHTML = `Weather Alerts in ${resp.county.split(', ')[0]}`;
                        get('#modal .popup-content').innerHTML = `<span id="close"></span>${alerts}`;
                    }
                }
            });
        }
    ...
    async getWWAs() {
        const json = await api(apiURL + 'wwas', [['filter', 'OR']]);

        this.displayWWAs(json);
...
function init() {
    ...
    map = new mapboxgl.Map({
        ...
        if (features != null && features.length > 0) {
            features.forEach((f) => {
                if (f.sourceLayer == 'mileposts-68fgu7') {
                    const name = ucwords(f.properties.HWYNAME.toLowerCase()),
                        num = f.properties.HWYNUMB,
                        mp = f.properties.MP,
                        id = f.properties.RDWY_ID,
                        dir = (hwy, id) => {
                            let d = '';
