let map,
    ...
    cloudBased = (windowHost == 'localhost' || windowHost == 'www.mapotechnology.com' ? true : false),
    key = (windowHost == 'www.mapotechnology.com' ? webkey : nativekey),
...
if (cloudBased) {
    var layers_roads = true,
        layers_webcams = true,
        layers_rwis = true,
        layers_incidents = true,
        layers_traffic = false,
        layers_construction = false,
        layers_vms = false,
        useMapCaching = true;
}
...
function findIncident(n, m = false) {
    ...
    if (bounds.length == 1) {
        map.setView([bounds[0].lat, bounds[0].lng], 11);
            ...
            onEachFeature: (feature, layer) => {
                ...
                layer.on('click', (e) => {
                    ...
                    if (cloudBased) {
                        ...
                        new RoadNetwork(e.latlng.lat, e.latlng.lng, report[0].hwyID.toString()).getRange(report);
                    } else {
...
function displayIncidents(json) {
    ...
    console.info('ODOT dispatch is reporting ' + json.features.length + ' incidents');
...
async function getConstruction() {
    if ((localStorage.getItem('construction') == null || storageIsOld('construction', 300)) || !useLocalStorage) {
        const resp = await fetch(apiURL + 'roads/incidents?key=' + key + '&rw=1');
...
function initialize() {
    /*if (typeof layers_roads !== 'undefined' &&
        ...
        basemap = L.tileLayer('https://www.mapotechnology.com/assets/images/tiles/5/{z}/{x}/{y}.png', {
            tileSize: 256,
            attribution: '&copy; ' + new Date().getFullYear() + ' Mapbox',
            token: mapboxToken,
            useCache: useMapCaching
        }).addTo(map);
