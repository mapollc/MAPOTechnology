...
function hash(t, id, name = null) {
    ...
    if (name != null) {
        const h = document.title.split(' | ')[1];
        document.title = name + ' | ' + (h == undefined ? document.title : h);
    }
}
...
function setRT(i) {
    radarImgs.forEach((e, n) => {
        var d = new Date(radarImgs[n].options.time * 1000),
            t = dow[d.getDay()].substring(0, 3) + ' ' + (d.getHours() > 12 ? d.getHours() - 12 : d.getHours()) + ':' + (d.getMinutes() < 10 ? '0' : '') + d.getMinutes() + ' ' + (d.getHours() >= 12 ? 'P' : 'A') + 'M';

        if (n == i) {
            document.querySelector('.radar-controls #time').innerHTML = t;
            e.setOpacity(0.8);
            radarCount = i;
        } else {
            e.setOpacity(0);
...
class RoadNetwork {
    ...
    async retrieve() {
        if (localStorage.getItem('road_network') == null) {
            ...
            const resp = await fetch('https://www.mapotechnology.com/apps/oreroads/mileposts.geojson');
    ...
    getRange(report, rn) {
        if (report.length == 0 || report == null) {
            /*createDialog('No Report', 'Currently, there is no road report for ' + rn + '.');*/
        } else {
            let i = 0,
                mps = [],
                arr = [],
            ...
            p.getLayers().forEach((f) => {
                if (f._latlng) {
                    ...
                    if ((ll.lat >= minlat && ll.lat <= maxlat) && (ll.lng >= minlon && ll.lng <= maxlon)) {
                        ...
                        } else if (n == 2 && f.feature != undefined) {
    ...
    displayFeatures(report) {
        ...
        report.rwis.forEach((r, i) => {
            ...
            b += '<h2 style="align-items:center">' + roadName(r.properties.station.name) + genFav('rwis', r.properties.station.id, r.properties.station.name) + '</h2><span class="updated">Last report ' + timeAgo(r.properties.updated) + '</span><div class="rows">' +
        ...
        report.rw.forEach((r, i) => {
            c += '<h2 style="align-items:center"><span class="rc" style="background-color:' + road(r.road.id) + '"></span>' + r.name + ' (' + roadName(r.hwy) + ')' + genFav('roadReports', r.name, r.name) + '</h2>' +
            ...
            '<div class="line"><div class="de">New Snow</div><span>' + r.snow.new + ' in</span></div>' +
            '<div class="line"><div class="de">Roadside Snow</div><span>' + r.snow.roadside + ' in</span></div>' +
        ...
        report.incidents.forEach((f, i) => {
            ...
            d += '<h2 style="align-items:left"><i class="fas fa-' + ty + '" style="color:' + col + ';margin-right:0.75em"></i>' + data.type + '</h2>' +
        ...
        modal.querySelector('.content[data-tab=rw]').innerHTML = (c ? c : '');
...
class Modal {
    ...
    vms() {
        ...
        modal.style.display = 'flex';
        hash('dms', this.json.id, this.json.name);
    }
    ...
    roadReport(name, int = 0) {
        if (this.json.length == 0) {
            ...
            rows += '<div class="line"><div class="de">New Snow</div><span>' + (data.snow.new == null ? 'N/A' : data.snow.new + ' in') + '</span></div>';
            rows += '<div class="line"><div class="de">Roadside Snow</div><span>' + (data.snow.roadside == null ? 'N/A' : data.snow.roadside + ' in') + '</span></div>';
            ...
            modal.style.display = 'flex';
            hash('rw', data.id, data.name + ' (' + roadName(data.hwy) + ')');
        }
    }
    ...
    rwis() {
        ...
        modal.style.display = 'flex';
        hash('rwis', data.station.id, roadName(data.station.name));
    }
    ...
    incident() {
        ...
        if (data.type == 'Crash') {
            ...
            ty = 'do-not-enter';
            col = '#dc3545';
        } else if (data.category == 'Road Work') {
            ty = 'traffic-cone';
            col = '#f19333';
        } else {
            ty = 'diamond-exclamation';
        ...
        modal.style.display = 'flex';
        hash('incident', data.id, 'Incident #' + data.id);
    }
    ...
    webcam() {
        ...
        modal.style.display = 'flex';
        hash('cam', this.json.id, this.json.name);
    }
    ...
    if (hash != '') {
        ...
        var int = setInterval(() => {
            ...
            if (htype == 'incident' && incidents.getLayers().length != 0) {
                ...
                construction.getLayers().forEach((e) => {
                    if (e.feature !== undefined && e.feature.properties.id == id) {
                        exists = true;
...
/* verify if the user has a subscription */
document.onreadystatechange = (e) => {
    if (document.readyState === 'complete') {
        validSubscription = function () {
            let valid = false,
                now = new Date().getTime() / 1000,
                liveSub = 'price_1OcY8LIpCdpJm6cT7zCxwKi4',
                testSub = 'price_1OcYB3IpCdpJm6cTvstK4y8N';

            if (subscribe && subscribe.active) {
                if (subscribe.plan == testSub) {
                    if (now >= subscribe.start && now <= subscribe.ends) {
                        valid = true;
                    } else {
                        console.info('Subscription has expired');
                    }
                } else {
                    console.info('Subscription is active, but not the correct plan');
                }
            }
        ...
        console.info('Subscription: ' + validSubscription());
    }
};
...
window.addEventListener('load', () => {
    ...
    if (window.location.search.search('logout') >= 0) {
    ...
    get('#user').addEventListener('click', () => {
        ...
        if (user) {
            document.body.insertAdjacentHTML('beforeend', '<ul id="userMenu"><li class="head"><span>Hi, ' + user.first_name + '</span></li><li onclick="syncFavorites(true)"><a href="#" onclick="return false">Sync Favorites</a></li><li><a href="' + host + 'account/settings">Account</a></li>' +
                ...
                '<li><a target="blank" href="../about/contact">Contact Us</a></li><li class="version">Version ' + version + '</li></ul>');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /*let verifySub = async function () {
        ...
        if (token != '') {
            const resp = await fetch(apiURL + 'user?key=' + key + '&mode=subs&token=' + token);
...
window.addEventListener('click', (e) => {
    ...
    if (e.target.id == 'close') {
        history.replaceState(null, null, ' ');
        document.title = (document.title.split(' | ').length > 1 ? document.title.split(' | ')[1] : document.title);
    ...
    if (e.target.id == 'result') {
        ...
        if (e.target.getAttribute('data-report')) {
            const r = roads.getLayers()[0].getLayers()[e.target.getAttribute('data-id')];

            if (r != null) {
                if (r._latlng) {
                    map.setView(r.getLatLng(), 12);
                } else {
...
window.addEventListener('change', (e) => {
    if (e.target.className == 'toggle') {
        ...
        if (e.target.getAttribute('data-layer') == 'radar') {
            if (e.target.checked) {
                getRadarAPI();
            } else {
                radarImgs.forEach((r) => {
                ...
                document.querySelector('.radar-controls #time').innerHTML = '--';
                document.querySelector('.radar-controls #radarTime').value = 0;
            }
        } else {
