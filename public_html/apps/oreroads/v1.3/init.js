        ...
        let intvl = setInterval(() => {
            if (router.type() == 'road-segment' && rwRetrieved && rwisRetrieved && incidentsRetrieved && roadWorkRetrieved && dmsRetrieved && webcamsRetrieved) {
                const data = JSON.parse(atob(theID)),
                    report = new Data().compileReports(data.name);
                ...
                roads.features.forEach((rw) => {
                    if (rw.properties.id == theID) {
                        s = counterpart(rw.properties.name.toLowerCase());
                        reports.push(rw.properties);
                    }
                });

                roads.features.forEach((rw) => {
                    if (rw.properties.name.toLowerCase() == s.toLowerCase()) {
                        reports.push(rw.properties);
                    }
                });
                ...
                rwis.forEach((f) => {
                    if (f.properties.station.id == theID) {
...
function removeHash() {
    const newUrl = window.location.origin + '/oregonroads' + window.location.hash;
    window.history.pushState({}, siteTitle, newUrl);
    document.title = siteTitle;
}

function progress(m) {
    if (isVisible(get('#loading'))) {
        /*get('#loading').querySelector('span').innerHTML = m;*/
    }
}
...
function hideLoading() {
    ...
    if (get('.logo') && !isVisible(get('.logo'))) {
...
function goToRWIS(id) {
    rwis.forEach((stn) => {
        if (stn.properties.station.id == id) {
            stn.properties.station = JSON.stringify(stn.properties.station);
            stn.properties.surface = JSON.stringify(stn.properties.surface);
            stn.properties.weather = JSON.stringify(stn.properties.weather);
...
class Router {
    constructor(qp) {
        ...
        this.route = [];
        this.params = this.params.split('#')[0].split('/');
