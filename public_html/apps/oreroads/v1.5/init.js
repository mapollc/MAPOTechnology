let queryParams = window.location.href.split('oregonroads/')[1];
...
function nearestCity(a, b) {
    const theCity = [],
        theDist = [],
        theBear = [];

    cities.list.forEach((c) => {
        const dist = distance(c.lat, c.lon, a, b);
        const bear = getBearing(c.lat, c.lon, a, b);
        theCity.push(c.city);
        theDist.push(dist);
        theBear.push(bear);
    });
...
function findIncident(id, callback) {
    ...
    incidents.filter(f => f.properties.id == id).forEach(f => {
